 ...
 * Uses the `withdraw_funds` RPC for atomic balance update + transaction recording.
 *
...
export async function POST(request: Request) {
  try {
    ...
    const description = String(body.description ?? "Withdrawal")
    ...
    if (amount > 1_000_000) {
      return NextResponse.json(
        { error: "Withdrawal amount exceeds maximum of KSh 1,000,000" },
        { status: 400 }
      )
    }

    // Call the atomic withdrawal RPC
