...
export function BetSlip({ mobile = false, onClose }: { mobile?: boolean; onClose?: () => void }) {
  ...
  const { user, isAuthenticated } = useAuth(); const [message, setMessage] = useState("")
  const router = useRouter()
  async function submit() {
    if (!isAuthenticated || !user) return router.push("/register?next=/")
    if (!selections.length || multiStake <= 0) return setMessage("Add selections and enter a stake to continue.")
    const wallet = await getWallet(user.id)
    if (wallet.main < multiStake) return router.push("/wallet?intent=deposit&next=/")
    const bet = await placeBet(); if (!bet) return setMessage("Add selections and enter a stake to continue.")
    try { await createBet(user.id, bet); setMessage("Bet placed successfully.") } catch { setMessage("Bet recorded locally. Connect the bets table to persist it.") }
  }
  return <aside className={mobile ? "fixed inset-x-3 bottom-3 z-[60] max-h-[75vh] overflow-auto rounded-2xl border border-border bg-card p-4 shadow-2xl" : "sticky top-20 hidden h-fit rounded-2xl border border-border bg-card p-4 shadow-lg xl:block"}>
    ...
    <div className="mt-4 space-y-2 border-t border-border pt-3 text-sm"><div className="flex justify-between"><span className="text-muted-foreground">Total odds</span><b>{(combinedOdds ?? 1).toFixed(2)}</b></div><div className="flex justify-between"><span className="text-muted-foreground">Possible win</span><b className="text-primary">{formatKes(potentialReturn)}</b></div></div>{message && <p className="mt-3 text-xs text-muted-foreground">{message}</p>}<Button className="mt-4 w-full font-bold" onClick={submit}>Place Bet</Button>{selections.length > 0 && <button onClick={clearSlip} className="mt-2 w-full text-xs text-muted-foreground hover:text-foreground">Clear Slip</button>}</aside>
}
