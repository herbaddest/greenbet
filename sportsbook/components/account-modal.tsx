...
export function AccountModal({ type, onClose }: { type: "deposit" | "withdraw"; onClose: () => void }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      const result = isDeposit
        ? await depositFunds(Number(amount), `MPESA-${Date.now()}`, `M-Pesa deposit from ${phone}`)
        : await withdrawFunds(Number(amount), `MPESA-${Date.now()}`, `M-Pesa withdrawal to ${phone}`)
  ...
  return <div className="fixed inset-0 z-[70] grid place-items-center bg-black/70 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label={isDeposit ? "Deposit" : "Withdraw"}>
    <form onSubmit={submit} className="glass w-full max-w-md rounded-2xl p-6 shadow-2xl">
      ...
      <div className="mt-6 flex gap-3"><Button type="button" variant="outline" className="flex-1" onClick={onClose} disabled={loading}>Cancel</Button><Button className="flex-1" disabled={loading || success}>{loading ? <><Loader2 className="mr-2 size-4 animate-spin" /> Processing...</> : success ? "Done" : "Continue"}</Button></div>
    </form>
  </div>
}
